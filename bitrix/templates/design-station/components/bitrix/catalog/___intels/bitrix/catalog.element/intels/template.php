...
echo "<div class=\"basket pad {$basketBlockWidth}\">
							...
							<div class=\"h2\">";
								if ($arResult["PRICES"]["BASE"]["DISCOUNT_VALUE"] != 0) {
									echo $arResult["PRICES"]["BASE"]["DISCOUNT_VALUE"]."<span class='r'>Р</span>.";
								} else {
                  ...
                  if ($qntw!=1) {
                    $dataqw = "data-minqw=\"".$qntw."\"";
									...
									echo "<div class=\"qty \" {$dataqw}>
											<input class=\"quantity h2  text-center\" type=\"text\" name=\"{$arParams["PRODUCT_QUANTITY_VARIABLE"]}\" value=\"{$qntw}\" /><div class='qty-ch'><div class=\"up\"></div><div class=\"down\"></div></div><input onmousedown=\"try { rrApi.addToBasket({$arResult["ID"]}) } catch(e) {}\" class=\"intoCart button reverse\" name=\"{$arParams["ACTION_VARIABLE"]}ADD2BASKET\" type=\"submit\" value=\"Купить\" />
										</div>";
								}
