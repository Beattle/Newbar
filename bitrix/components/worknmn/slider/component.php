...
if(!CModule::IncludeModule("worknmn.slider")) {
	$this->AbortResultCache();
	ShowError("WORKNMN_SLIDER_MODULE_NOT_INSTALLED");
	return false;
}
...
if($arParams['IBLOCK_ID'] > 0 AND $arParams['IBLOCK_ID2'] > 0)
{
	$arrFilter["IBLOCK_ID"] = array(0 => $arParams["IBLOCK_ID"], 1 => $arParams['IBLOCK_ID2']);
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))){
	...
	$arResult = CWorknmnSlider::getData($arParams, $arrFilter);
  $arResult['iblock_prods'] = $arParams['IBLOCK_ID2'];  
  $arResult['iblock_slider'] = $arParams['IBLOCK_ID'];
	$this->SetResultCacheKeys(array("IDS"));
	$this->IncludeComponentTemplate();
}

CWorknmnSlider::includeLibs($arParams);