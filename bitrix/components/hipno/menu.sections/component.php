...
if($this->StartResultCache())
{
	...
	else
	{
        ...
        $arSelect = array("ID","DEPTH_LEVEL","NAME","SECTION_PAGE_URL","IBLOCK_SECTION_ID");

        if(  isset($arParams["COL"]) && isset($arParams['ROW']))
        {
            array_push($arSelect,$arParams['COL'],$arParams['ROW']);
        }
		...
		while($arSection = $rsSections->GetNext())
		{
			...
			$arResult['SECTIONS'][] = array(
                ...
                "~UF_COL" => $arSection["~UF_COL"],
                "~UF_ROW" => $arSection["~UF_ROW"],
                "PARENT_ID" =>is_null($arSection["~IBLOCK_SECTION_ID"]) ? NULL :$arSection["~IBLOCK_SECTION_ID"],
			);
            if(is_null($arSection['IBLOCK_SECTION_ID'])){
                $arResult['PARENT_ID'][] = $arSection['ID'];
            }
...
{
	if ($menuIndex > 0)
		$arResult[$menuIndex - 1]["IS_PARENT"] = $arSection["DEPTH_LEVEL"] > $previousDepthLevel;
	$previousDepthLevel = $arSection["DEPTH_LEVEL"];

	$arResult["ELEMENT_LINKS"][$arSection["ID"]][] = urldecode($arSection["~SECTION_PAGE_URL"]);
	$arResult[$menuIndex++] = array(
        ...
        'COLUMN' => $arSection['~UF_COL'],
        'ROW' => $arSection['~UF_ROW'],
	);
}

unset($arResult['SECTIONS']);
unset($arResult['ELEMENT_LINKS']);
unset($arResult['PARENT_ID']);

$arResult = buildTree($arResult);



Foreach($arResult as $key => &$branch){
       $cols = array();
       $rows = array();
    for ($i=0,$k = count($branch['children']);$i<$k;$i++){
        $cols[$i] = $branch['children'][$i]['COLUMN'];
        $rows[$i] = $branch['children'][$i]['ROW'];
    }
    if(!empty($cols) && !empty($rows)){
        array_multisort($cols,SORT_ASC,$rows,SORT_ASC,$branch['children']);
    }
