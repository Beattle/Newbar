...
foreach($arParams["FIELD_CODE"] as $field_code)
{
	...
	switch ($field_code)
	{
		...
		case "SECTION_ID":
			...
			$field_res .= InputType("checkbox", $_name, "Y", $_value, false, "", "")."&nbsp;".GetMessage("CC_BCF_INCLUDE_SUBSECTIONS");
		...
		case "L":
			...
			if ('C' == $arProp['LIST_TYPE'])
			{
				...
				if ('Y' == $arProp['MULTIPLE'])
				{
					...
					{
						$arListRadio[] = '<td class="cell-filter"><input id="'.$key.'" type="checkbox" name="'.$name.'[]" value="'.htmlspecialcharsbx($key).'"'.(in_array($key, $arListValue) ? ' checked' : '').'/><label for="'.$key.'"> '.htmlspecialcharsex($val).'</label></td>';
					}
				}
				else
				{
