...
if (empty($arRunErrors))
{
	...
	{
		$arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "ACTIVE", "ACTIVE_FROM", "ACTIVE_TO", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "LANG_DIR", "DETAIL_PAGE_URL", "PROPERTY_POSTAVKA", "PROPERTY_DESCTIUBLIZKO");
    
    // было для яндекса
		//$filter = Array("IBLOCK_ID"=>$IBLOCK_ID, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "PROPERTY_MARKET_VALUE"=>"Y");
    $filter = Array("IBLOCK_ID"=>$IBLOCK_ID, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
      ...
      // можно ошибки вывести 
      //$arRunErrors[] = print_r($arAcc);
      // анализируем в наличии или нет товар
      if (($arAcc["PROPERTY_POSTAVKA_VALUE"] != "")AND(strtolower($arAcc["PROPERTY_POSTAVKA_VALUE"]) != "в наличии"))
        $str_AVAILABLE = ' available="false"';
          ...
          //$arRunErrors[] = print_r($arAcc);
            ...
            /*
						yandex_text2xml(TruncateText(
							...
							255), true)
            PROPERTY_POSTAVKA_VALUE DESCTIUBLIZKO*/
            (is_array($arAcc["PROPERTY_DESCTIUBLIZKO_VALUE"])?
						yandex_text2xml(TruncateText(
							($arAcc["PROPERTY_DESCTIUBLIZKO_VALUE"]["TYPE"]=="html"?
							strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["PROPERTY_DESCTIUBLIZKO_VALUE"]["TEXT"])) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["PROPERTY_DESCTIUBLIZKO_VALUE"]["TEXT"])),
							255), true)
            : yandex_text2xml($arAcc["NAME"], true))
            //yandex_text2xml($arAcc["NAME"], true)
						."</description>\n";
					break;
				case 'param':
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
			...
			if ('X' == $arCatalog['CATALOG_TYPE'] && !$boolItemOffers)
			{
				...
				if (($str_QUANTITY <= 0) && ($str_QUANTITY_TRACE == "Y"))
					...
					$str_AVAILABLE = ' available="true"';
        //$arRunErrors[] = print_r($arProduct);  
        //if ($arProduct["POSTAVKA"] != "")
        //  $str_AVAILABLE = ' available="false"';
        //$strTmpOff = "123123";
        //$str_AVAILABLE = ' available="false"';
